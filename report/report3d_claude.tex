...
\end{equation}
Recall that this is the friction exerted on the element by the inner layer, which resists the element from sliding forward with $x$. The components of this friction force are equivalent to the shear stresses acting on the inner ($-\hat r$) face. The magnitude of the Coulomb friction force is $\mu$ times the normal pressure $-\sigma_{rr}$ on that face; writing it this way presumes $\sigma_{rr}\le0$ (the element is under compression), which we verify self-consistently once the system is solved (Section~\ref{sec:closed_form}). Therefore,
\begin{align}
    \sigma_{ur}\left(\zeta = -\frac{t}{2}\right) &= -\mu \sigma_{rr}\left(\zeta = -\frac{t}{2}\right)\sin(2\phi), \label{eq:sigma_ur_inner}\\
    \sigma_{vr}\left(\zeta = -\frac{t}{2}\right) &= -\mu \sigma_{rr}\left(\zeta = -\frac{t}{2}\right)\cos(2\phi). \label{eq:sigma_vr_inner}
\end{align}
...
\begin{align}
    \sigma_{ur}\left(\zeta = \frac{t}{2}\right) &= -\mu \sigma_{rr}\left(\zeta = \frac{t}{2}\right)\sin(2\phi), \label{eq:sigma_ur_outer}\\
    \sigma_{vr}\left(\zeta = \frac{t}{2}\right) &= -\mu \sigma_{rr}\left(\zeta = \frac{t}{2}\right)\cos(2\phi). \label{eq:sigma_vr_outer}
\end{align}
...
By Newton's third law, two surfaces in contact experience equal and opposite traction; because their outward normals point in opposite directions, this means their normal stresses must agree. It suffices to match $\sigma_{rr}$ alone, since the transverse shears at each surface are already tied to it by Eqs.~\eqref{eq:sigma_ur_inner}--\eqref{eq:sigma_vr_outer}. This gives a recursive relationship for the boundary values of $\sigma_{rr}$, using the same piecewise gates as Eq.~\eqref{eq:overlap_in}--\eqref{eq:overlap_out}:
\begin{align}
    \text{Outer surface:}&\quad \sigma_{rr}\left(u,v,\zeta=\frac{t}{2}\right) = \begin{cases}
        \sigma_{rr}\left(u^+,v^+,\zeta=-\frac{t}{2}\right) & v \ge -\frac{W}{2}+2\pi r\sin\phi\\
        0 &\text{otherwise}
    \end{cases}\label{eq:contact_outer}\\
    \text{Inner surface:}&\quad \sigma_{rr}\left(u,v,\zeta=-\frac{t}{2}\right) = \begin{cases}
        \sigma_{rr}\left(u^-,v^-,\zeta=\frac{t}{2}\right) & v \le \frac{W}{2}-2\pi r\sin\phi\\
        0 &\text{otherwise}
    \end{cases}\label{eq:contact_inner}
\end{align}
(the outer face of this element touches the \emph{inner} face of its outer neighbor at $(u^+,v^+)$, and the inner face touches the \emph{outer} face of its inner neighbor at $(u^-,v^-)$ --- note the two relations reference opposite neighbors, matching Section~\ref{sec:neighbors}). Equations~\eqref{eq:contact_outer}--\eqref{eq:contact_inner} recursively relate the boundary stress of one layer to that of its neighbor, one full turn away; propagated far enough in $u$, they reach the outermost wound layer at $u=L$, past which no material is wound at all (Section~\ref{sec:neighbors}), so
\begin{equation}
    \sigma_{rr}\left(u=L,v,+\frac{t}{2}\right)=0 \quad\text{for all }v,
\end{equation}
regardless of the $v$-condition in Eq.~\eqref{eq:contact_outer} --- there is simply nothing at $(u^+,v^+)$ to be in contact with. This is the base case that starts the recursion. Note that Eqs.~\eqref{eq:contact_outer}--\eqref{eq:contact_inner} are pure \emph{matching} conditions between neighbors: they say nothing yet about what value $\sigma_{rr}$ actually takes away from this base case. Section~\ref{sec:closed_form} supplies the piece that does.


\section{PDE formulation and closed-form solution}\label{sec:closed_form}
We now assemble the pieces above into a closed system for the stress field. The three equilibrium equations, Eqs.~\eqref{eq:inplane_u_full}--\eqref{eq:outofplane_r_full}, are three PDEs for six unknown stress components at every point $(u,v,\zeta)$. Section~\ref{sec:friction} supplies what is needed to close this at the two surfaces $\zeta=\pm\frac{t}{2}$: there, $\sigma_{ur},\sigma_{vr}$ are fixed directly in terms of $\sigma_{rr}$ by Coulomb friction (Eqs.~\ref{eq:sigma_ur_inner}--\ref{eq:sigma_vr_outer}), and $\sigma_{rr}$'s own two boundary values are tied recursively, layer to layer, by Newton's third law (Eqs.~\ref{eq:contact_outer}--\ref{eq:contact_inner}). Combined with the linear-through-thickness approximation of Section~\ref{sec:equilibrium}, this reduces every unknown to a function of $(u,v)$ alone. We also carry forward the free-edge boundary conditions on the long, unloaded edges of the ribbon,
...
\subsection{Thickness integration}\label{sec:thickness_integration}
